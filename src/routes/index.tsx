...
function HeroSection({ onSelectTab }: { onSelectTab: (tab: TabId) => void }) {
  return (
...
function AboutSection({ onSelectTab, showBack = false }: { onSelectTab: (tab: TabId) => void; showBack?: boolean }) {
  return (
    <section id="about" className="relative flex min-h-[85vh] items-center overflow-hidden rounded-t-[40px] bg-portfolio-dark px-5 py-24 sm:rounded-t-[52px] sm:px-8 md:px-10">
      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col items-center text-center">
          ...
          {showBack && (
            <div className="mb-6 flex justify-center">
              <button
                ...
                className="rounded-full border border-portfolio-line bg-portfolio-surface px-4 py-2 text-xs font-semibold uppercase tracking-wider text-portfolio-muted hover:text-portfolio-text"
              >
                ← Back Home
              </button>
            </div>
        ...
        <FadeIn delay={0.15}>
          <div className="mt-8 flex flex-wrap justify-center gap-3 text-xs uppercase tracking-wider text-portfolio-muted">
        ...
        <FadeIn delay={0.2}>
          <div className="mt-12 flex flex-wrap justify-center gap-4">
            ...
            <button
              onClick={() => onSelectTab("projects")}
              className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-portfolio-line bg-portfolio-surface px-6 py-3 text-xs font-semibold uppercase tracking-widest text-portfolio-text transition-colors hover:bg-portfolio-line sm:text-sm"
            >
              Explore Projects
            </button>
          </div>
...
function SkillsSection({ onSelectTab, showBack = false }: { onSelectTab?: (tab: TabId) => void; showBack?: boolean }) {
  return (
    <section id="skills" className="rounded-t-[40px] bg-portfolio-light px-5 py-20 text-portfolio-light-ink sm:rounded-t-[52px] sm:px-8 sm:py-24 md:px-10 md:py-32">
      <FadeIn>
        {showBack && onSelectTab && (
          <div className="mb-6 flex justify-center">
            <button
              onClick={() => onSelectTab("home")}
              className="rounded-full border border-portfolio-light-line bg-white px-4 py-2 text-xs font-semibold uppercase tracking-wider text-portfolio-light-muted hover:text-portfolio-light-ink"
            >
              ← Back Home
            </button>
          </div>
...
function ProjectsSection({ onSelectTab, showBack = false }: { onSelectTab?: (tab: TabId) => void; showBack?: boolean }) {
  ...
  return (
    <section id="projects" className="relative -mt-10 rounded-t-[40px] bg-portfolio-dark px-5 py-24 sm:-mt-12 sm:rounded-t-[52px] sm:px-8 md:px-10 md:py-32">
      <FadeIn>
        {showBack && onSelectTab && (
          <div className="mb-6 flex justify-center">
            <button
      ...
      <div className="mx-auto mt-14 max-w-6xl space-y-12 sm:mt-20">
              ...
              <div className="flex h-full flex-col justify-between rounded-2xl border border-portfolio-line bg-portfolio-surface p-6 transition-all hover:border-portfolio-text">
                <div>
                  <div className="grid size-11 place-items-center rounded-xl border border-portfolio-line bg-portfolio-dark text-portfolio-accent">
                    {item.iconName === "Code2" && <Code2 className="size-5" />}
                    {item.iconName === "Layers" && <Layers className="size-5" />}
                    {item.iconName === "Cpu" && <Cpu className="size-5" />}
                  </div>
...
function AchievementsSection({ onSelectTab, showBack = false }: { onSelectTab?: (tab: TabId) => void; showBack?: boolean }) {
  return (
    <section id="achievements" className="bg-portfolio-dark px-5 py-20 sm:px-8 md:px-10">
      <div className="mx-auto max-w-6xl">
        <FadeIn>
          ...
          <p className="mt-3 max-w-xl text-sm font-light text-portfolio-muted">
            Key milestones achieved through competitions, freelance web development, and practical problem solving.
          </p>
        </FadeIn>
        ...
        <div className="mt-10 grid gap-5 sm:grid-cols-2">
          {achievements.map((item, i) => (
            <FadeIn key={item.title} delay={i * 0.1}>
              <div className="flex h-full flex-col justify-between rounded-2xl border border-portfolio-line bg-portfolio-surface p-7 transition-all hover:border-portfolio-text">
                <div>
                  <div className="flex items-center justify-between">
                    <span className="rounded-full bg-portfolio-accent/10 px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-portfolio-accent">
                      {item.tag}
                    </span>
                  ...
                  <h3 className="mt-4 text-xl font-bold uppercase tracking-tight text-portfolio-text">
                    {item.title}
                  </h3>
                  <p className="mt-2 text-sm font-light leading-relaxed text-portfolio-muted">
                    {item.description}
        ...
        <div className="mt-16 border-t border-portfolio-line pt-12">
          <FadeIn>
            ...
            <h3 className="mt-2 text-2xl font-bold uppercase tracking-tight text-portfolio-text">
              Certifications
            </h3>
          </FadeIn>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {certifications.map((cert, i) => (
              <FadeIn key={cert.title} delay={i * 0.1}>
                <div className="flex items-center justify-between rounded-xl border border-portfolio-line bg-portfolio-surface p-5">
                  <div className="flex items-center gap-3">
                    <div className="grid size-10 place-items-center rounded-lg bg-portfolio-dark text-portfolio-accent">
                      <GraduationCap className="size-5" />
                    </div>
                    <div>
                      <h4 className="font-semibold text-portfolio-text text-sm sm:text-base">{cert.title}</h4>
                      <p className="text-xs text-portfolio-muted uppercase tracking-wider">{cert.date}</p>
                    </div>
                  </div>
                </div>
              </FadeIn>
...
function EducationSection({ onSelectTab, showBack = false }: { onSelectTab?: (tab: TabId) => void; showBack?: boolean }) {
  return (
    <section id="education" className="bg-portfolio-dark px-5 py-20 sm:px-8 md:px-10">
      <div className="mx-auto max-w-5xl">
        <FadeIn>
          ...
          <p className="mt-3 max-w-xl text-sm font-light text-portfolio-muted">
            Formal Computer Science and Engineering education and prior academic foundation.
          </p>
        </FadeIn>
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl font-bold uppercase tracking-tight text-portfolio-text sm:text-3xl">
                      {edu.degree}
                    </h3>
...
function ContactSection({ onSelectTab, showBack = false }: { onSelectTab?: (tab: TabId) => void; showBack?: boolean }) {
  return (
    <section id="contact" className="rounded-t-[40px] bg-portfolio-light px-5 py-20 text-portfolio-light-ink sm:rounded-t-[52px] sm:px-8 sm:py-24 md:px-10 md:py-32">
      <div className="mx-auto max-w-5xl">
        <FadeIn>
          {showBack && onSelectTab && (
            <div className="mb-6 flex justify-center">
          ...
          <p className="mx-auto mt-4 max-w-xl text-center text-sm font-light text-portfolio-light-muted sm:text-base">
            {personal.contactCta}
          </p>
        </FadeIn>

        <FadeIn delay={0.1}>
          <div className="mt-14 overflow-hidden rounded-3xl border border-portfolio-light-line bg-white p-8 shadow-sm sm:p-12">
            <div className="grid gap-8 md:grid-cols-2">
              <div>
                <h3 className="text-2xl font-bold uppercase tracking-tight text-portfolio-light-ink">
                  Let&apos;s Connect
                </h3>
                <p className="mt-2 text-sm font-light leading-relaxed text-portfolio-light-muted">
                  Feel free to reach out directly regarding software engineering opportunities, internships, or web development projects.
                </p>

                <div className="mt-8 space-y-4">
                  <a
                    href={`mailto:${personal.email}`}
                    className="flex items-center gap-3 text-sm font-medium text-portfolio-light-ink transition-colors hover:text-portfolio-accent sm:text-base"
                  >
                    <div className="grid size-10 place-items-center rounded-full bg-portfolio-light text-portfolio-accent">
                      <Mail className="size-4" />
                    </div>
              ...
              <div className="flex flex-col justify-between rounded-2xl border border-portfolio-light-line bg-portfolio-light p-6 sm:p-8">
                <div>
                  <h4 className="text-sm font-bold uppercase tracking-widest text-portfolio-light-ink">
                    Links & Profiles
                  </h4>
                  <div className="mt-4 flex flex-wrap gap-3">
                    <a
                      ...
                      rel="noreferrer"
                      aria-label="GitHub Profile"
                      className="inline-flex items-center gap-2 rounded-full border border-portfolio-light-line bg-white px-4 py-2 text-xs font-semibold uppercase tracking-wider text-portfolio-light-ink shadow-xs transition-colors hover:bg-portfolio-light-line"
                    >
                ...
                <div className="mt-8 border-t border-portfolio-light-line pt-6">
                  <ResumeButton label="Open Full Google Docs CV" className="w-full justify-center" />
                </div>
              </div>
            </div>
          </div>
...
function HomeView({ onSelectTab }: { onSelectTab: (tab: TabId) => void }) {
  return (
    <div className="flex flex-col">
      ...
      <FloatingRibbon />
      <AboutSection onSelectTab={onSelectTab} />
      <SkillsSection onSelectTab={onSelectTab} />
      <ProjectsSection onSelectTab={onSelectTab} />
      <CurrentlyLearningSection />
      <AchievementsSection onSelectTab={onSelectTab} />
      <EducationSection onSelectTab={onSelectTab} />
      <ContactSection onSelectTab={onSelectTab} />
    </div>
  );
}
    ...
    <main className="min-h-screen overflow-x-clip bg-portfolio-dark font-sans text-portfolio-text antialiased selection:bg-portfolio-accent selection:text-white">
      ...
      <header className="sticky top-0 z-50 border-b border-portfolio-light-line/60 bg-white/95 px-4 py-3 backdrop-blur-md transition-colors sm:px-8">
        ...
        >
          ...
          <button
            ...
            className="flex cursor-pointer items-center gap-2 text-portfolio-accent"
          >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {activeTab === "about" && <AboutSection onSelectTab={handleSelectTab} showBack />}
          {activeTab === "skills" && <SkillsSection onSelectTab={handleSelectTab} showBack />}
          {activeTab === "projects" && <ProjectsSection onSelectTab={handleSelectTab} showBack />}
          {activeTab === "achievements" && <AchievementsSection onSelectTab={handleSelectTab} showBack />}
          {activeTab === "education" && <EducationSection onSelectTab={handleSelectTab} showBack />}
          {activeTab === "contact" && <ContactSection onSelectTab={handleSelectTab} showBack />}
        </motion.div>
      </AnimatePresence>
